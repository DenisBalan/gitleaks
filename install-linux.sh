...
get_latest_release() {
  ...
  curl --fail -sS "${headers[@]}" "https://api.github.com/repos/gitleaks/gitleaks/releases/latest" | # Get latest release from GitHub api
    grep '"tag_name":' |                                                                                    # Get tag line
    sed -E 's/.*"([^"]+)".*/\1/'                                                                            # Pluck JSON value
}

download_path=$(mktemp -d -t gitleaks.XXXXXXXXXX)
download_zip="${download_path}/gitleaks.zip"
download_executable="${download_path}/gitleaks"

if [ -z "${GITLEAKS_VERSION}" ] || [ "${GITLEAKS_VERSION}" == "latest" ]; then
...
curl --fail -sS -L -o "${download_zip}" "https://github.com/gitleaks/gitleaks/releases/download/${version}/gitleaks_${version:1}_${os}.tar.gz"
...
if [[ $os == "windows"* ]]; then
  dest="${GITLEAKS_INSTALL_PATH:-/bin}/"
  ...
  dest="${GITLEAKS_INSTALL_PATH:-/usr/local/bin}/"
  echo "Installing ${download_executable} to ${dest} ..."
